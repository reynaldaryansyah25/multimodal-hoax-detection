...
def load_existing_metadata(path="./data/raw/youtube/metadata.json"):
    """Load existing metadata and return records + set of video_ids"""
    if os.path.exists(path):
        try:
            with open(path, 'r', encoding='utf-8') as f:
                records = json.load(f)
            video_ids = {r['video_id'] for r in records if 'video_id' in r}
...
def main(keywords=SEARCH_KEYWORDS, max_per_keyword=30, 
         date_start="2024-10-20", date_end=None, target_total=1000):
    ...
    print(f"\n{'='*70}")
    print(f"RESUME MODE: Starting from {current_count} existing videos")
    print(f"Target total: {target_total}")
    print(f"Need to collect: {max(0, target_total - current_count)} more videos")
    ...
    for kw_idx, keyword in enumerate(keywords, 1):
        ...
        print(f"Progress: {len(records)}/{target_total}")
        print(f"{'='*70}")
        ...
        for vid_idx, video in enumerate(videos, 1):
            ...
            if video_id in existing_video_ids:
                print(f"\n[{vid_idx}/{len(videos)}] SKIP: Already downloaded - {video['title'][:60]}")
                stats['skipped_duplicate'] += 1
                continue
            
            sample_id = f"YT_{len(records)+1:05d}"
            ...
            try:
                ...
                if not is_valid_duration(duration):
                    print(f"  Duration invalid ({duration//60 if duration else 0}m)")
                ...
                print(f"  SUCCESS ({len(records)}/{target_total})")
