...
def extract_thumbnail_accurate(html_content, url, source_name):
    """Extract thumbnail dengan prioritas meta tags dan manual selector"""
    soup = BeautifulSoup(html_content, 'html.parser')
    
    # PRIORITAS 1: og:image meta tag (paling reliable untuk thumbnail)
    og_image = soup.find('meta', property='og:image')
    if og_image and og_image.get('content'):
        img_url = og_image.get('content')
        # Filter iklan detik
        if 'detik.com' in url and 'ads' not in img_url.lower() and 'iklan' not in img_url.lower():
            return img_url
        elif 'detik.com' not in url:
            return img_url
    ...
    if tw_image and tw_image.get('content'):
        ...
        if 'ads' not in img_url.lower() and 'iklan' not in img_url.lower():
            return img_url
    
    # PRIORITAS 3: Manual extraction per portal (lebih spesifik)
    if source_name == 'detik':
        # Detik: ambil dari detail__media atau dalam artikel
        selectors = [
            '.detail__media-image',
            '.detail__body-image img',
            'img.detail__media-image',
            'figure.detail__media img'
        ]
        for selector in selectors:
            img = soup.select_one(selector)
            if img:
                src = img.get('src') or img.get('data-src')
                if src and 'ads' not in src.lower() and 'logo' not in src.lower():
                    return src
    
    elif source_name == 'kompas':
        # Kompas: photo__img atau read__img
        selectors = [
            '.photo__img',
            '.read__img',
            'img.photo__img',
    ...
    # PRIORITAS 4: First large image in article (minimal width 300px)
    article_imgs = soup.select('article img, .article-content img, .detail-content img')
    for img in article_imgs:
        src = img.get('src') or img.get('data-src')
        if src and 'ads' not in src.lower() and 'logo' not in src.lower():
            # Check width attribute kalau ada
...
def scrape_article(url, source_name):
    """Scrape article dengan improved thumbnail extraction"""
    try:
        ...
        # IMPROVED: Extract thumbnail dengan manual method
        thumbnail = extract_thumbnail_accurate(article.html, url, source_name)
        
        # Fallback ke newspaper4k kalau manual gagal
        if not thumbnail:
...
def download_image(url, path):
    ...
    try:
        ...
        ad_keywords = ['ads', 'iklan', 'banner', 'adv', 'advertisement']
        if any(kw in url.lower() for kw in ad_keywords):
            return ""
        ...
        # Check file size (skip jika terlalu kecil = mungkin logo/icon)
        content_length = r.headers.get('content-length')
        if content_length and int(content_length) < 5000:  # 5KB minimum
            return ""
...
def scrape_news(
        ...
        for i, future in enumerate(as_completed(future_to_url), 1):
            ...
            try:
                ...
                if result:
                    ...
                    img_status = "🖼️" if result.get('image') else "❌"
    ...
    if download_images and articles:
        ...
        print(f"\n  Success: {success}/{len(articles)} thumbnails downloaded")
